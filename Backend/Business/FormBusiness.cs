...
namespace Business
{
    ...
    public class FormBusiness : JwtBaseBusiness<Form, FormDTO> ,IBusiness<FormDTO, FormDTO>
    {
        ...
        public async Task<FormDTO> UpdateAsync(FormDTO formDTO)
        {
            if (formDTO.Id <= 0)
            {
                _logger.LogWarning("Se intentó Actualizar un Form con ID inválido: {FormId}", formDTO.Id);
                throw new Utilities.Exceptions.ValidationException("id", "El ID del Form debe ser mayor que cero");
            }
            ...
            try
            {
                ...
                var updateForm = await _formData.UpdateAsync(existingForm);

                return _mapper.Map<FormDTO>(updateForm);
            }
            catch (Exception ex) 
            {
